...
| A05 | Security Misconfiguration | NestJS `ValidationPipe` with `whitelist: true` strips unexpected payload fields on every endpoint; verbose stack traces disabled outside development (`NODE_ENV=production`); default/example credentials never shipped in seed data for production — the one exception is `prisma/seed.ts`'s two demo users (used for manual verification and the Task 9 e2e test), which is now gated to refuse running when `NODE_ENV=production`. |
